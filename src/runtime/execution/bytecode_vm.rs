...
impl RuntimeSession for BytecodeRuntimeSession {
    ...
    fn invoke(
        ...
        let res = self
            .vm
            .run("main") // This should be updated to run specific function if needed
    ...
    match val {
        ...
        RuntimeValue::Object(obj_rc) => {
            let vm_obj = obj_rc
                ...
                .unwrap_or_else(|e| e.into_inner())
                .iter()
                .map(|(k, v)| (k.clone(), rt_to_vm_value(v.clone())))
                .collect();
...
fn vm_to_rt_value(val: VmValue) -> RuntimeValue {
    match val {
        ...
        VmValue::Array(arr) => Value::array(arr.into_iter().map(vm_to_rt_value).collect()),
        VmValue::Object(obj) => Value::object(
            obj.into_iter()
                .map(|(k, v)| (k, vm_to_rt_value(v)))
                .collect(),
        ),
        _ => RuntimeValue::Null,
    }

...
pub fn spawn_actor_native(vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if args.is_empty() {
        return Err(EvalError::new("spawn(entry_func, args...) expects at least a function name".to_string()));
    }
    
    let entry_func = match &args[0] {
        Value::Str(s) => s.clone(),
        _ => return Err(EvalError::new("First argument must be the entry function name".to_string())),
    };
    
    let func_args = if args.len() > 1 { args[1..].to_vec() } else { vec![] };
    ...
    std::thread::spawn(move || {
        ...
        if let Err(_) = result {
            log::error!("[Nyx-Concurrent] Actor {} panicked! Isolation maintained.", id);
        }
...
pub fn send_actor_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if args.len() < 2 {
        return Err(EvalError::new("send(actor_id, message) expects 2 arguments".to_string()));
    }
    
    let target_id = match args[0] {
        Value::Int(i) => i as u64,
        _ => return Err(EvalError::new("First argument must be an integer Actor ID".to_string())),
    };
    ...
    if let Some(tx) = registry.get(&target_id) {
        ...
        match tx.try_send(msg) {
            ...
            Err(std::sync::mpsc::TrySendError::Full(_)) => {
                log::warn!("[Nyx-Concurrent] Actor {} mailbox is full! Message dropped.", target_id);
                Ok(Value::Bool(false))
            }
...
pub fn receive_actor_native(vm: &mut NyxVm, _args: &[Value]) -> Result<Value, EvalError> {
    if let Some(mailbox_lock) = &vm.actor_mailbox {
        ...
        Err(EvalError::new("Current context does not have an actor mailbox.".to_string()))
    }
}

...
impl VirtualDevice for ConsoleDevice {
    fn device_type(&self) -> DeviceType { DeviceType::Console }
...
impl VirtualDevice for BlockDevice {
    ...
    fn name(&self) -> &str { &self.name }
    fn read(&mut self, _port: u16, _size: usize) -> DeviceResult<u64> { Ok(0) }
    fn write(&mut self, _port: u16, _s: usize, _v: u64) -> DeviceResult<()> { Ok(()) }
    fn interrupt(&mut self, _irq: u8) -> DeviceResult<()> { Ok(()) }
    fn as_any_mut(&mut self) -> &mut dyn std::any::Any { self }
}
...
impl NetworkDevice {
    pub fn new() -> Self { Self { name: "virtio-net".to_string() } }
}
...
impl SerialDevice {
    pub fn new() -> Self { Self { name: "serial".to_string() } }
}
...
impl PicDevice {
    pub fn new() -> Self { Self { name: "i8259-pic".to_string() } }
}
...
impl VirtualDevice for PicDevice {
    ...
    fn name(&self) -> &str { &self.name }
    fn read(&mut self, _port: u16, _size: usize) -> DeviceResult<u64> { Ok(0) }
    fn write(&mut self, _port: u16, _p: usize, _v: u64) -> DeviceResult<()> { Ok(()) }
    fn interrupt(&mut self, _irq: u8) -> DeviceResult<()> { Ok(()) }
    fn as_any_mut(&mut self) -> &mut dyn std::any::Any { self }
}
...
pub fn create_standard_devices(config: &super::vm::VmConfig) -> DeviceManager {
    ...
    let blk0_pci = Arc::new(Mutex::new(super::pci::VirtioBlockPci::new_with_inner(blk0, 0x1000)));
    ...
    if let Some(iso_path) = &config.iso {
        if !iso_path.is_empty() && iso_path != "None" {
            if let Ok(cd) = super::virtio_block::VirtioBlock::new(iso_path) {
                let cd_pci = Arc::new(Mutex::new(super::pci::VirtioBlockPci::new_with_inner(cd, 0x2000)));

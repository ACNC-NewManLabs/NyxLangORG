...
use nyx_diagnostics::{ErrorCategory, NyxError};
use nyx_vm::hypervisor::{Architecture, VirtualMachine, VmConfig as LowLevelVmConfig};
...
impl Hypervisor {
    ...
    pub fn new(config: HypervisorConfig) -> Result<Self, NyxError> {
        ...
        let vm = VirtualMachine::new(vm_config)
            .map_err(|e| NyxError::new("H001", e, ErrorCategory::Runtime))?;
        Ok(Self {
    ...
    pub fn run(self) -> Result<(), NyxError> {
...

        {
            ...
            if let Some((data, entry)) = kernel_info {
                vm.load_kernel(&data, entry)
                    .map_err(|e| NyxError::new("H004", e, ErrorCategory::Runtime))?;
                println!("Nyx Hypervisor: Kernel loaded (PC=0x{:x}).", entry);
            } else {
                println!("Nyx Hypervisor: Starting in BIOS-only mode (Reset Vector=0xFFFF0).");
            }
            vm.start()
                .map_err(|e| NyxError::new("H005", e, ErrorCategory::Runtime))?;
        }
        ...
        loop {
            ...
            // ── Rendering ─────────────────────────────────────────────────
            let close_x1 = TOTAL_W - 40;
            let btn_y1 = 8;
            let min_x1 = TOTAL_W - 72;
            let ss_x1 = 12;
            let ss_x2 = SIDEBAR_W - 12;
            let ss_y1 = 190;
            let ss_y2 = 218;

            let hovering_close =
                mx >= close_x1 && mx <= (close_x1 + 28) && my >= btn_y1 && my <= (btn_y1 + 20);
            let _hovering_min =
                mx >= min_x1 && mx <= (min_x1 + 28) && my >= btn_y1 && my <= (btn_y1 + 20);
            let hovering_ss = mx >= ss_x1
                && mx <= ss_x2
                && my >= (TITLEBAR_H + ss_y1)
                && my <= (TITLEBAR_H + ss_y2);
            ...
            if frame_count % 500 == 0 {
                ...
                if active_pixels > 1000 {
                    println!(
                        "Nyx Hypervisor: UI Health Check [OK] ({} active pixels)",
                        active_pixels
                    );
                } else {
                    ...
                    if fb.len() == VM_W * VM_H {
                        for row in 0..VM_H {
                            ...
                            if (dst_start + VM_W) <= buf.len() {
                                buf[dst_start..dst_start + VM_W]
                                    .copy_from_slice(&fb[src_start..src_start + VM_W]);
                            }
                        }
                        blitted = true;
                    }
            ...
            if screenshot_requested {
                let path = format!(
                    "/home/surya/Nyx Programming Language/nyx_boot_log_{}.png",
                    screenshot_count
                );
...
fn save_screenshot(buf: &[u32], w: usize, h: usize, path: &str) {
    ...
    println!(
        "Nyx Hypervisor: Saving screenshot to {} ({}x{})...",
        path, w, h
    );
    match img.save(path) {

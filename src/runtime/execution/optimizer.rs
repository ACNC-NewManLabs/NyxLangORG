    ...
    fn optimize(&self, plan: LogicalPlan) -> LogicalPlan {
        match plan {
            LogicalPlan::Filter { input, predicate } => {
                ...
                if let LogicalPlan::Projection { input: inner_input, exprs, names } = input {
            ...
            LogicalPlan::Projection { input, exprs, names } => {
                let input = self.optimize(*input);
                if let LogicalPlan::Scan { source_id, projection: _, options, schema } = input {
                    ...
                    LogicalPlan::Projection { input: Box::new(input), exprs, names }
                }
            }

...
impl VirtualDevice for VirtioBlock {
    ...
    fn name(&self) -> &str { "virtio-block" }
    fn read(&mut self, _port: u16, _size: usize) -> DeviceResult<u64> { Ok(0) }
    fn write(&mut self, _port: u16, _size: usize, _value: u64) -> DeviceResult<()> { Ok(()) }
...
impl super::virtio::VirtioDevice for VirtioBlock {
    fn device_id(&self) -> u32 { 2 } // VirtIO Block
    fn config_size(&self) -> usize { 8 }
    fn reset(&mut self) {}
    
    fn notify(&mut self, _q_idx: u32, q: &mut super::virtio::VirtioQueue, mem: &mut super::memory::VirtualMemory) {
        while let Some(head_idx) = q.get_next_avail(mem) {
             ...
             let addr = mem.read_phys(super::memory::GuestPhysicalAddr(desc_table_ptr), 8).unwrap_or(0);
             ...
             let sector = u64::from_le_bytes([head_buf[8], head_buf[9], head_buf[10], head_buf[11], head_buf[12], head_buf[13], head_buf[14], head_buf[15]]);
             
             // 2. Read/Write Data
             // (Simplified: assume next descriptor is data)
             let next_idx = mem.read_phys(super::memory::GuestPhysicalAddr(desc_table_ptr + 14), 2).unwrap_or(0) as u16;
             let next_desc_ptr = q.desc_table + (next_idx as u64 * 16);
             let data_addr = mem.read_phys(super::memory::GuestPhysicalAddr(next_desc_ptr), 8).unwrap_or(0);
             let data_len = mem.read_phys(super::memory::GuestPhysicalAddr(next_desc_ptr + 8), 4).unwrap_or(0) as u32;

             match req_type {
                 0 => { // READ (VIRTIO_BLK_T_IN)
                     let mut buf = vec![0u8; data_len as usize];
                     let _ = self.read_sectors(sector, &mut buf);
                     let _ = mem.write_buf(super::memory::GuestPhysicalAddr(data_addr), &buf);
                 }
                 1 => { // WRITE (VIRTIO_BLK_T_OUT)
                     let mut buf = vec![0u8; data_len as usize];
             ...
             let status_idx = mem.read_phys(super::memory::GuestPhysicalAddr(next_desc_ptr + 14), 2).unwrap_or(0) as u16;
             let status_desc_ptr = q.desc_table + (status_idx as u64 * 16);
             let status_addr = mem.read_phys(super::memory::GuestPhysicalAddr(status_desc_ptr), 8).unwrap_or(0);

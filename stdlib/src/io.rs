pub mod io {
    ...
    impl<R: Read> Reader<R> {
        ...
        pub fn read_to_string(&mut self) -> Result<NyxString, NyxError> {
            ...
            self.inner.read_to_string(&mut s).map_err(|e| {
                NyxError::new("IO001", format!("Read failure: {}", e), ErrorCategory::Io)
                    .with_suggestion(
                        "Check if the stream is closed or if you have necessary permissions.",
                    )
            })?;
            Ok(NyxString::from(&s))
        }
    ...
    impl<W: Write> Writer<W> {
        ...
        pub fn write_all(&mut self, buf: &[u8]) -> Result<(), NyxError> {
            self.inner.write_all(buf).map_err(|e| {
                NyxError::new("IO003", format!("Write failure: {}", e), ErrorCategory::Io)
        ...
        pub fn flush(&mut self) -> Result<(), NyxError> {
            self.inner.flush().map_err(|e| {
                NyxError::new("IO004", format!("Flush failure: {}", e), ErrorCategory::Io)
            })
        }
    }

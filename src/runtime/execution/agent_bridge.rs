...
lazy_static! {
    static ref VECTOR_COLLECTIONS: RwLock<HashMap<String, VectorCollection>> = RwLock::new(HashMap::new());
}
...
pub fn cosine_similarity_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if args.len() < 2 {
        return Err(EvalError::new("cosine_similarity expects two arrays".to_string()));
    }
    
    let a = match &args[0] {
        Value::DoubleArray(rc) => rc.read().unwrap_or_else(|e| e.into_inner()).clone(),
        Value::Array(rc) => rc.read().unwrap_or_else(|e| e.into_inner()).iter()
            .map(|v| v.as_f64().unwrap_or(0.0)).collect(),
        _ => return Err(EvalError::new("First argument must be an array of numbers".to_string())),
    };
    ...
    let b = match &args[1] {
        ...
        Value::Array(rc) => rc.read().unwrap_or_else(|e| e.into_inner()).iter()
            .map(|v| v.as_f64().unwrap_or(0.0)).collect(),
        _ => return Err(EvalError::new("Second argument must be an array of numbers".to_string())),
    };
...
pub fn vector_create_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    let name = match args.first() {
        ...
        _ => return Err(EvalError::new("vector_create(name) expects a string".to_string())),
    };
    let mut store = VECTOR_COLLECTIONS.write().unwrap_or_else(|e| e.into_inner());
    store.insert(name, VectorCollection { 
        vectors: Vec::new(), 
        gpu_vectors: None,
        dim: 0,
        dirty: true,
...
pub fn vector_insert_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if args.len() < 2 {
        return Err(EvalError::new("vector_insert(collection, vector) expects 2 arguments".to_string()));
    }
    let name = match &args[0] {
        Value::Str(s) => s.clone(),
        _ => return Err(EvalError::new("First argument must be a collection name (string)".to_string())),
    };
    ...
    if coll.dim == 0 {
        ...
        return Err(EvalError::new(format!("Dimension mismatch: collection expects {}, got {}", coll.dim, vector.len())));
    }
...
pub fn vector_clear_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    let name = match args.first() {
        Some(Value::Str(s)) => s.clone(),
        _ => return Err(EvalError::new("vector_clear(name) expects a string".to_string())),
    };
    let mut store = VECTOR_COLLECTIONS.write().unwrap_or_else(|e| e.into_inner());
...
pub fn vector_search_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if args.len() < 3 {
        return Err(EvalError::new("vector_search(collection, query, k) expects 3 arguments".to_string()));
    }
    ...
    let query_vec: Vec<f64> = match &args[1] {
        ...
        Value::Array(rc) => rc.read().unwrap_or_else(|e| e.into_inner()).iter()
            .map(|v| v.as_f64().unwrap_or(0.0)).collect(),
        _ => return Err(EvalError::new("Query must be an array of numbers".to_string())),
    };
    ...
    match &args[0] {
        Value::Str(name) => {
            if let Some(coll) = store.get_mut(name) {
                ...
                let results = if use_gpu {
                    log::info!("[Nyx-Agent] Dispatching GPU Search for {} vectors", num_vectors);
                    gpu_vector_search(coll, &query_vec, k)?
                } else {
                    let res_f64 = simd_kernels::simd_f64_vector_search(&coll.vectors, &query_vec, coll.dim, k);
                    res_f64.into_iter().map(|(id, score)| (id, score as f32)).collect()
                };
                ...
                Err(EvalError::new(format!("Collection '{}' does not exist", name)))
            }
        },
        Value::Array(matrix_rc) => {
            ...
            if dim == 0 { return Ok(Value::Array(Arc::new(RwLock::new(Vec::new())))); }
        ...
        _ => Err(EvalError::new("Collection must be a name (string) or an array of vectors".to_string())),
    }
}
...
fn ensure_gpu_sync(coll: &mut VectorCollection) -> Result<Arc<NyxBuffer>, EvalError> {
    ...
    let (device, queue) = gpu_bridge::ensure_gpu()
        .ok_or_else(|| EvalError::new("GPU not initialized"))?;

    let f32_data: Vec<f32> = coll.vectors.iter().map(|&x| x as f32).collect();
    let size_bytes = (f32_data.len() * 4) as u64;
    
    let buf = gpu_bridge::acquire_storage_buffer(device, size_bytes, &format!("VectorCollection_{}x{}", coll.vectors.len()/coll.dim, coll.dim))
...
fn gpu_vector_search(coll: &mut VectorCollection, query: &[f64], k: usize) -> Result<Vec<(usize, f32)>, EvalError> {
    let (device, queue) = gpu_bridge::ensure_gpu()
        .ok_or_else(|| EvalError::new("GPU not initialized"))?;
    ...
    let query_buf = gpu_bridge::acquire_storage_buffer(device, (query_f32.len() * 4) as u64, "Search Query")
        .ok_or_else(|| EvalError::new("Failed to acquire GPU query buffer"))?;
    queue.write_buffer(&query_buf, 0, bytemuck::cast_slice(&query_f32));

    let scores_buf = gpu_bridge::acquire_storage_buffer(device, (num_vectors * 4) as u64, "Search Scores")
    ...
    // 2. Setup Pipeline
    let ps = SEARCH_PIPELINE.get_or_init(|| {
        ...
        Some(SearchPipeline { pipeline: pipe, bind_group_layout: bgl })
    }).as_ref().ok_or_else(|| EvalError::new("Failed to initialize GPU pipeline"))?;

    let bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
        layout: &ps.bind_group_layout,
        entries: &[
            wgpu::BindGroupEntry { binding: 0, resource: matrix_buf.inner.as_ref().unwrap().as_entire_binding() },
            wgpu::BindGroupEntry { binding: 1, resource: query_buf.inner.as_ref().unwrap().as_entire_binding() },
            wgpu::BindGroupEntry { binding: 2, resource: scores_buf.inner.as_ref().unwrap().as_entire_binding() },
...
pub fn chat_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    ...
    let response = format!("Nyx-v1.0 AI Response to: '{}'. [LLM Engine operational]", prompt);

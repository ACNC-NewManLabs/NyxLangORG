...
use super::cpu::{CpuEmulator, Register, Architecture};
...
impl JitEngine {
    ...
    pub fn compile_block(&mut self, cpu: &mut CpuEmulator, guest_pc: u64) -> Result<*const u8, String> {
        ...
        let _isa_builder = cranelift_native::builder().unwrap().finish(settings::Flags::new(flag_builder)).unwrap();
        ...
        let count = builder.ins().load(ir::types::I64, ir::MemFlags::new(), counter_ptr, 0);
        let new_count = builder.ins().iadd_imm(count, 1);
        builder.ins().store(ir::MemFlags::new(), new_count, counter_ptr, 0);

        // Get or create hit counter for this block
        self.hit_counters.entry(guest_pc).or_insert_with(|| Arc::new(AtomicU64::new(0)));

        // Register mapping helpers
        let get_reg_offset = |reg: Register| -> i32 {
            (reg as i32) * 8
        };
                ...
                Architecture::X86_64 => {
                    match opcode {
                        0x21 => { // AND r/m64, r64
                            ...
                            let v1 = builder.ins().load(ir::types::I64, ir::MemFlags::new(), cpu_ptr, rax_off);
                            let v2 = builder.ins().load(ir::types::I64, ir::MemFlags::new(), cpu_ptr, rbx_off);
                            let res = builder.ins().band(v1, v2);
                            builder.ins().store(ir::MemFlags::new(), res, cpu_ptr, rax_off);
                            current_pc += 3;
                        }
                        0x09 => { // OR r/m64, r64
                            let rax_off = get_reg_offset(Register::Rax);
                        ...
                        0x01 => { // ADD r/m64, r64
                            ...
                            builder.ins().store(ir::MemFlags::new(), sum, cpu_ptr, rax_off);
                            current_pc += 3;
                        }
                        0x29 => { // SUB r/m64, r64
                            let rax_off = get_reg_offset(Register::Rax);
                            ...
                            builder.ins().store(ir::MemFlags::new(), diff, cpu_ptr, rax_off);
                            current_pc += 3;
                        }
                        0x39 => { // CMP r/m64, r64
                            let rax_off = get_reg_offset(Register::Rax);
                            ...
                            builder.ins().store(ir::MemFlags::new(), zf_val, cpu_ptr, zf_off);
                            current_pc += 3;
                        }
                        0xD3 => { // SHL/SHR r/m64, CL
                            let rax_off = get_reg_offset(Register::Rax);
                            let rcx_off = get_reg_offset(Register::Rcx);
                            let val = builder.ins().load(ir::types::I64, ir::MemFlags::new(), cpu_ptr, rax_off);
                            let cl  = builder.ins().load(ir::types::I64, ir::MemFlags::new(), cpu_ptr, rcx_off);
                            let amt = builder.ins().ireduce(ir::types::I8, cl);
                            let res = builder.ins().ishl(val, amt);
                            builder.ins().store(ir::MemFlags::new(), res, cpu_ptr, rax_off);
                            current_pc += 3;
                        }
                        0xB8..=0xBF => { // MOV reg, imm32
                            ...
                            let reg = [Register::Rax, Register::Rcx, Register::Rdx, Register::Rbx,
                                       Register::Rsp, Register::Rbp, Register::Rsi, Register::Rdi][reg_idx as usize];
                            let offset = get_reg_offset(reg);
                            let val = builder.ins().iconst(ir::types::I64, imm as i64);
                            builder.ins().store(ir::MemFlags::new(), val, cpu_ptr, offset);
                            current_pc += 5;
                        }
                        0x74 => { // JZ rel8
                            let offset = cpu.read_memory(current_pc + 1, 1).map_err(|e| e)? as i8;
                            let zf_off = 1433i32;
                            let zf = builder.ins().load(ir::types::I8, ir::MemFlags::new(), cpu_ptr, zf_off);
                            let is_zero = builder.ins().icmp_imm(ir::condcodes::IntCC::NotEqual, zf, 0);
                            ...
                            builder.ins().store(ir::MemFlags::new(), next_pc, cpu_ptr, rip_off);
                            builder.ins().return_(&[]);
                            break;
                        }
                        0x75 => { // JNZ rel8
                            ...
                            let zf_off = 1433i32;
                            let zf = builder.ins().load(ir::types::I8, ir::MemFlags::new(), cpu_ptr, zf_off);
                            let is_not_zero = builder.ins().icmp_imm(ir::condcodes::IntCC::Equal, zf, 0);
                            let true_pc  = (current_pc as i64 + 2 + offset as i64) as u64;
                            let false_pc = current_pc + 2;
                            let rip_off  = get_reg_offset(Register::Rip);
                            let true_val  = builder.ins().iconst(ir::types::I64, true_pc as i64);
                        ...
                        0xC3 => { // RET
                            let rip_off = get_reg_offset(Register::Rip);
                            let next_pc = builder.ins().iconst(ir::types::I64, (current_pc + 1) as i64);
                            builder.ins().store(ir::MemFlags::new(), next_pc, cpu_ptr, rip_off);
                            builder.ins().return_(&[]);
                            break;
                        }
        ...
        let id = self.module.declare_function(&format!("block_{:x}", guest_pc), Linkage::Export, &ctx.func.signature)

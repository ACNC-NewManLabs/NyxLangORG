...
impl NyxBlockStorage {
    ...
    pub fn write_chunk(&self, table_name: &str, chunk: &DataChunk) -> std::io::Result<u64> {
        ...
        let serialized = bincode::serialize(chunk).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
        let header = PageHeader {
            magic: *b"NYXB",
            version: 1,
            chunk_size: serialized.len() as u32,
            row_count: chunk.size as u32,
            checksum: 0, // TODO: Implement CRC
        };

        let start_pos = file.seek(SeekFrom::End(0))?;
        file.write_all(&bincode::serialize(&header).unwrap())?;
    ...
    pub fn read_chunks(&self, table_name: &str) -> std::io::Result<Vec<DataChunk>> {
        ...
        loop {
            ...
            let header: PageHeader = bincode::deserialize(&header_buf).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;

            let mut body_buf = vec![0u8; header.chunk_size as usize];
            file.read_exact(&mut body_buf)?;
            let chunk: DataChunk = bincode::deserialize(&body_buf).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
            chunks.push(chunk);
        }

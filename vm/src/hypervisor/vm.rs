...
use super::cpu::{Architecture, CpuEmulator, CpuState, Register};
use super::devices::{
    create_standard_devices, DeviceManager, VirtualDevice, FB_BASE, FB_HEIGHT, FB_SIZE, FB_WIDTH,
};
...
impl VirtualMachine {
    ...
    pub fn new(config: VmConfig) -> Result<Self, String> {
            ...
            match KvmContext::new(config.memory, config.num_cpus) {
                Ok(mut ctx) => {
                    ...
                    if !ctx.vcpu_fds.is_empty() {
                        ...
                        ctx.vcpu_fds[0]
                            .set_cpuid2(&cpuid)
                            .map_err(|e| format!("Failed to set CPUID: {}", e))?;
                    }
        ...
        if let Some(cmos_boxed) = vm.devices.get_device_mut("cmos") {
            if let Some(cmos) = cmos_boxed
                .as_any_mut()
                .downcast_mut::<super::cmos::CmosDevice>()
            {
    ...
    pub fn update_mouse(&mut self, x: u32, y: u32, buttons: u32) {
        ...
        let _ = self
            .memory
            .write_phys(GuestPhysicalAddr(base + 4), 4, y as u64);
        let _ = self
            .memory
            .write_phys(GuestPhysicalAddr(base + 8), 4, buttons as u64);
    }
    ...
    pub fn load_bios(&mut self) {
        ...
        if mem.len() >= bios_base + bios_data.len() {
            ...
            println!(
                "Nyx Hypervisor: Nyx BIOS loaded successfully ({} bytes at 0x{:x})",
                len, bios_base
            );
            ...
            eprintln!(
                "Warning: Guest memory too small to load 128KB BIOS (Size: {} bytes)",
                mem.len()
            );
        }
    }
                        ...
                        match jit.compile_block(&mut cpu, pc) {
                            Ok(code_ptr) => {
                                ...
                                if let Some(counter) = jit.hit_counters.get(&pc) {
                                    ...
                                    type JitFunc =
                                        unsafe extern "C" fn(*mut super::cpu::CpuState, *const u64);
                ...
                loop {
                    match ctx.run_vcpu(0) {
                        ...
                        Ok(kvm_ioctls::VcpuExit::MmioRead(addr, data)) => {
                            ...
                            if let Ok(val) =
                                self.memory.read_phys(GuestPhysicalAddr(addr), data.len())
                            {
                        ...
                        Ok(kvm_ioctls::VcpuExit::MmioWrite(addr, data)) => {
                            ...
                            let _ =
                                self.memory
                                    .write_phys(GuestPhysicalAddr(addr), data.len(), val);
    ...
    fn check_hypercall(&mut self, cpu_idx: usize) -> Result<bool, String> {
        ...
        let (opcode, _rax) = {
            let mut cpu = self.cpus[cpu_idx].lock().unwrap();
            (
                cpu.read_memory(pc, 1).unwrap_or(0) as u8,
                cpu.state.read_gpr(Register::Rax),
            )
        };
            ...
            Architecture::X86_64 => {
                if opcode == 0xF1 {
                    ...
                    if hypercall_num == 100 {
                        ...
                        println!(
                            "Nyx Hypervisor: Guest requested GetFbPtr -> 0x{:x}",
                            FB_BASE
                        );
                    ...
                    if hypercall_num == 101 {
                        ...
                        println!(
                            "Nyx Hypervisor: Guest requested GetInputPtr -> 0x{:x}",
                            input_ptr
                        );
    ...
    fn handle_io(&mut self, cpu_idx: usize) -> Result<bool, String> {
           ...
           opcode == 0xEE || opcode == 0xEF
        {
            ...
            match opcode {
                0xE6 | 0xE7 | 0xEE | 0xEF => {
                    // OUT
                    let port = if opcode >= 0xEE {
                        state.read_gpr(super::cpu::Register::Rdx) as u16
                    } else {
                        self.memory.read_phys(GuestPhysicalAddr(pc + 1), 1)? as u16
                    };
    ...
    pub fn save_snapshot(&self, path: &str) -> Result<(), String> {
        ...
        // Collect CPU states
        let cpu_states: Vec<CpuState> = self
            .cpus
            .iter()
            .map(|c| c.lock().unwrap().state.clone())
            .collect();
        ...
        let snapshot = (&self.config, cpu_states, memory_dump);

        bincode::serialize_into(&mut encoder, &snapshot).map_err(|e| e.to_string())?;
    ...
    pub fn load_snapshot(path: &str) -> Result<Self, String> {
        ...
        let (config, cpu_states, memory_dump): (
            VmConfig,
            Vec<CpuState>,
            super::memory::PageAlignedBuffer,
        ) = bincode::deserialize_from(&mut decoder).map_err(|e| e.to_string())?;
    ...
    #[cfg(feature = "jit")]
    #[allow(dead_code)]
    fn execute_jit_block(
        &self,
        code_ptr: *const u8,
        jit: &super::jit::JitEngine,
        pc: u64,
        cpu: &mut super::cpu::CpuEmulator,
    ) {
        if let Some(counter) = jit.hit_counters.get(&pc) {
            let counter_ptr = counter.as_ref() as *const _ as *const u64;
                        ...
                        for bit in 0..64 {
                            if (byte & (1 << bit)) != 0 {
                                ...
                                if offset + 4096 <= mem.len() {
                                    delta
                                        .insert(offset as u64, mem[offset..offset + 4096].to_vec());
                                }
                            }
                        }
                    }
...
mod tests {
    ...
    fn test_vm_run() {
        ...
        vm.memory
            .write_phys(GuestPhysicalAddr(0x1000), 1, 0xF4)
            .unwrap();

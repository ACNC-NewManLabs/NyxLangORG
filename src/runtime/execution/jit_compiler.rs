...
static KERNEL_CACHE: std::sync::OnceLock<
    RwLock<std::collections::HashMap<String, Arc<FusedKernel>>>,
> = std::sync::OnceLock::new();

pub fn get_or_create_fused_kernel(
    signature: &str,
    builder_fn: impl FnOnce() -> FusedKernel,
) -> Arc<FusedKernel> {
    ...
    cache
        ...
        .insert(signature.to_string(), kernel.clone());
    kernel
}

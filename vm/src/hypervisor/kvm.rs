...
impl KvmContext {
    ...
    pub fn new(_memory_size: u64, num_cpus: usize) -> Result<Self, String> {
        ...
        let vm_fd = kvm.create_vm().map_err(|e| format!("Failed to create VM: {}", e))?;
    ...
    #[cfg(target_os = "linux")]
    pub fn map_memory(&self, slot: u32, guest_addr: u64, memory: &Arc<Mutex<super::memory::PageAlignedBuffer>>) -> Result<(), String> {
    ...
    pub fn set_regs(&self, vcpu_idx: usize, regs: kvm_regs) -> Result<(), String> {
        ...
        vcpu.set_regs(&regs).map_err(|e| format!("Failed to set regs: {}", e))
    }
    ...
    pub fn set_sregs(&self, vcpu_idx: usize, sregs: kvm_sregs) -> Result<(), String> {
        let vcpu = &self.vcpu_fds[vcpu_idx];
        vcpu.set_sregs(&sregs).map_err(|e| format!("Failed to set sregs: {}", e))
    }
    ...
    pub fn get_regs(&self, vcpu_idx: usize) -> Result<kvm_regs, String> {
        let vcpu = &self.vcpu_fds[vcpu_idx];
        vcpu.get_regs().map_err(|e| format!("Failed to get regs: {}", e))
    }
    ...
    pub fn run_vcpu(&mut self, vcpu_idx: usize) -> Result<VcpuExit<'_>, String> {
        ...
        vcpu.run().map_err(|e| format!("KVM VCPU run failed: {}", e))
    }
    ...
    pub fn sync_regs_to_kvm(&self, vcpu_idx: usize, state: &CpuState) -> Result<(), String> {
        ...
        // Sync FPU/SSE
        let mut fpu = self.vcpu_fds[vcpu_idx].get_fpu().map_err(|e| e.to_string())?;
        for i in 0..16 { // KVM x86_64 has 16 XMM registers
            let v = state.vreg[i];
            let bytes = v.to_le_bytes();
            fpu.xmm[i].copy_from_slice(&bytes);
        }
        self.vcpu_fds[vcpu_idx].set_fpu(&fpu).map_err(|e| e.to_string())?;
    ...
    pub fn sync_regs_from_kvm(&self, vcpu_idx: usize, state: &mut CpuState) -> Result<(), String> {
        ...
        // Sync FPU/SSE back
        let fpu = self.vcpu_fds[vcpu_idx].get_fpu().map_err(|e| e.to_string())?;
        for i in 0..16 {
    ...
    pub fn get_dirty_log(&self, slot: u32, memory_size: u64) -> Result<Vec<u64>, String> {
        let dirty_log = self.vm_fd.get_dirty_log(slot, memory_size as usize)

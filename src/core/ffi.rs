...
impl NyxFFI {
    pub fn new() -> Self {
        Self {
            loaded_libraries: std::collections::HashMap::new(),
        }
    }
    ...
    pub unsafe fn get_symbol<T>(
        ...
        library: &str,
        symbol: &str,
    ) -> Result<libloading::Symbol<'_, T>, String> {
        let lib = self
            .loaded_libraries
            .get(library)
            .ok_or_else(|| "Library not loaded".to_string())?;
        let c_symbol = CString::new(symbol).map_err(|e| e.to_string())?;
        let func: libloading::Symbol<'_, T> = lib
            .get(c_symbol.as_bytes_with_nul())
...
mod tests {
    ...
    fn test_dynamic_ffi_invoke() {
        ...
        unsafe {
            ...
            ffi.load_library(math_lib)
                .expect(&format!("Failed to load {}", math_lib));

            // Resolve the C `cos` mathematical function: double cos(double x)
            type CosFunc = unsafe extern "C" fn(f64) -> f64;
            let cos_sym: libloading::Symbol<'_, CosFunc> = ffi
                .get_symbol(math_lib, "cos")

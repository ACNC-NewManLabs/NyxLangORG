...
use crate::index::{Symbol, GlobalIndex, SymbolKind as LspSymbolKind};
...
impl DocumentAnalyzer {
    ...
    pub fn new(uri: Url, source: &str) -> Self {
        ...
        let (functions, variables, types, struct_fields, variable_types, ast_cached) = match parser.parse(&tokens) {
            Ok(ast) => {
                ...
                for item in &ast.items {
                    match &item.kind {
                        ItemKind::Function(func) => {
                             let r = Range {
                                 start: Position { line: item.span.start.line as u32 - 1, character: item.span.start.column as u32 - 1 },
                                 end: Position { line: item.span.end.line as u32 - 1, character: item.span.end.column as u32 - 1},
                             };
            ...
            Err(_) => (HashMap::new(), HashMap::new(), HashMap::new(), HashMap::new(), HashMap::new(), None),
        };
            ...
            for tok in &self.tokens {
                 if tok.lexeme == identifier {
                      references.push(Location {
                            uri: self.uri.clone(),
                            range: Range {
                                start: Position { line: tok.span.start.line as u32 - 1, character: tok.span.start.column as u32 - 1 },
                                end: Position { line: tok.span.end.line as u32 - 1, character: tok.span.end.column as u32 - 1 },
                            },
                        });
                 }
            }
    ...
    pub fn get_members_at(&self, position: Position, index: &GlobalIndex) -> Vec<String> {
        ...
        if let Some(receiver) = self.get_identifier_at(line, if dot_idx > 0 { dot_idx - 1 } else { 0 }) {
    ...
    pub fn get_hover(&self, position: Position, index: &GlobalIndex) -> Option<Hover> {
        ...
        if let Some(identifier) = self.get_identifier_at(&line_str, position.character as usize) {
            ...
            for symbol in index.find(&identifier) {
                ...
                contents.push(MarkedString::String(format!("{} `{}` (global)", kind_str, identifier)));
...
fn extract_vars_and_types(stmts: &[Stmt], vars: &mut HashMap<String, Range>, v_types: &mut HashMap<String, String>) {
    for stmt in stmts {
        match stmt {
            Stmt::Let { name, var_type, span, .. } => {
                vars.insert(name.clone(), Range {
                     start: Position { line: span.start.line as u32 - 1, character: span.start.column as u32 - 1},
                     end: Position { line: span.end.line as u32 - 1, character: span.end.column as u32 - 1},
                });

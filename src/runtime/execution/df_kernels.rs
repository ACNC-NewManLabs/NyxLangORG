...
use crate::runtime::execution::df_engine::{self, DataChunk, Field, LogicalPlan, Schema};
...
pub fn generate_synthetic_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let (Some(Value::Int(rows)), Some(Value::Object(schema_map_rc))) =
        (args.first(), args.get(1))
    {
        ...
        for col in columns {
            ...
            let val_data = match &col.data {
                df_engine::ColumnData::F64(v) => {
                    Value::DoubleArray(Arc::new(RwLock::new((**v).clone())))
                }
                df_engine::ColumnData::I64(v) => Value::Array(Arc::new(RwLock::new(
                    v.iter().map(|&x| Value::Int(x)).collect(),
                ))),
                df_engine::ColumnData::Bitmap(bm) => {
                    let mut flags = Vec::with_capacity(bm.len);
                    for i in 0..bm.len {
                        flags.push(Value::Bool(bm.get(i)));
                    }
        ...
        Err(EvalError::new(
            "generate_synthetic(rows, {col: type}) expected".to_string(),
        ))
    }
}
                                ...
                                match socket.read(&mut buf).await {
                                    Ok(n) if n > 0 => {
                                        ...
                                        let (res_line, res_body) = if req.contains("GET /tables") {
                                            let catalog = df_engine::global_catalog()
                                                .lock()
                                                .unwrap_or_else(|e| e.into_inner());
...
pub fn col_filter_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let (Some(Value::Array(data)), Some(Value::Array(mask))) = (args.first(), args.get(1)) {
        ...
        let result: Vec<Value> = d
            .iter()
            .zip(m.iter())
            .filter_map(|(val, flag)| match flag {
...
pub fn col_min_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let Some(Value::Array(data)) = args.first() {
        let d = data.read().unwrap_or_else(|e| e.into_inner());
        let min = d.iter().filter_map(as_f64).fold(f64::INFINITY, f64::min);
        if min.is_infinite() {
...
pub fn rolling_sum_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let (Some(Value::Array(data)), Some(Value::Int(w))) = (args.first(), args.get(1)) {
        ...
        let result: Vec<Value> = (0..vals.len())
            .map(|i| {
                if i + 1 < window {
                    Value::Null
                } else {
                    Value::Float(vals[(i + 1 - window)..=i].iter().sum())
                }
            })
            .collect();
        return Ok(Value::Array(Arc::new(RwLock::new(result))));
        ...
        let result: Vec<Value> = (0..vals.len())
            .map(|i| {
                if i + 1 < window {
                    ...
                    Value::Float(vals[(i + 1 - window)..=i].iter().sum::<f64>() / wf)
                }
            })
            .collect();
        return Ok(Value::Array(Arc::new(RwLock::new(result))));
...
pub fn pearson_corr_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let (Some(Value::Array(a)), Some(Value::Array(b))) = (args.first(), args.get(1)) {
        ...
        let num: f64 = (0..n as usize)
            .map(|i| (av[i] - mean_a) * (bv[i] - mean_b))
            .sum();
        let den_a: f64 = (0..n as usize)
            .map(|i| (av[i] - mean_a).powi(2))
            .sum::<f64>()
            .sqrt();
        let den_b: f64 = (0..n as usize)
            .map(|i| (bv[i] - mean_b).powi(2))
            .sum::<f64>()
            .sqrt();
...
pub fn col_describe_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let Some(Value::Array(data)) = args.first() {
        ...
        map.insert(
            "min".to_string(),
            Value::Float(*sorted.first().unwrap_or(&0.0)),
        );
        ...
        map.insert(
            "max".to_string(),
            Value::Float(*sorted.last().unwrap_or(&0.0)),
        );
        return Ok(Value::Object(Arc::new(RwLock::new(map))));
    }
    Ok(Value::Null)
...
pub fn read_csv_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    ...
    let first_data = if data_start < lines.len() {
        lines[data_start]
    } else {
        return Ok(Value::Null);
    };
    ...
    let columns_arr: Vec<Value> = headers
        .iter()
        .zip(col_data)
        .map(|(name, data)| {
    ...
    result_map.insert(
        "columns".to_string(),
        Value::Array(Arc::new(RwLock::new(columns_arr))),
    );
...
pub fn write_csv_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    ...
    let col_names_arr = match args.first() {
        Some(Value::Array(a)) => a.clone(),
        _ => return Ok(Value::Bool(false)),
    };
    let col_data_arr = match args.get(1) {
        Some(Value::Array(a)) => a.clone(),
        _ => return Ok(Value::Bool(false)),
    };
    ...
    let nrows = match &cols[0] {
        Value::Array(a) => a.read().unwrap_or_else(|e| e.into_inner()).len(),
        _ => 0,
    };
    ...
    if let Some(Value::Array(data)) = args.first() {
        ...
        let result: Vec<Value> = order
            ...
            .map(|k| {
                ...
                m.insert(
                    "count".to_string(),
                    Value::Int(*counts.get(k).unwrap_or(&0)),
                );
...
pub fn col_normalize_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let Some(Value::Array(data)) = args.first() {
        ...
        let result: Vec<f64> = vals
            .iter()
            .map(|v| if range == 0.0 { 0.0 } else { (v - mn) / range })
            .collect();
        return Ok(floats_to_value_arr(result));
    }
    Ok(Value::Null)
...
pub fn encode_categorical_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let Some(Value::Array(data)) = args.first() {
        ...
        let mapping_val: Vec<Value> = mapping
            .iter()
            .map(|(k, v)| {
                let mut m = std::collections::HashMap::new();
        ...
        result.insert(
            "encoded".to_string(),
            Value::Array(Arc::new(RwLock::new(encoded))),
        );
        result.insert(
            "mapping".to_string(),
            Value::Array(Arc::new(RwLock::new(mapping_val))),
        );
...
pub fn scan_csv_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    ...
    for (k, v) in options {
        opt_nyx.insert(k, Value::Str(v));
    }
    obj.insert(
        "options".to_string(),
        Value::Object(Arc::new(RwLock::new(opt_nyx))),
    );
...
pub fn execute_plan_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let Some(plan_val) = args.first() {
        ...
        let mut ctx = df_engine::ExecutionContext {
            sources: HashMap::new(),
        };
        ...
        if let Some(Value::Object(sources_rc)) = args.get(1) {
            ...
            for (id, val) in sources.iter() {
                if let Value::Object(df_rc) = val {
                    ...
                    ctx.sources.insert(
                        id.clone(),
                        Box::new(df_engine::MemoryDataSource {
                            chunks: vec![chunk],
                            cursor: 0,
                            schema,
                        }),
        ...
        let mut physical_plan =
            df_engine::create_physical_plan(&logical_plan, &mut ctx).map_err(|e| EvalError {
                message: e,
                stack: vec![],
            })?;

        let mut all_chunks = Vec::new();
        while let Some(chunk) = physical_plan.next_chunk().map_err(|e| EvalError {
            message: e,
            stack: vec![],
        })? {
    ...
    Err(EvalError {
        message: "execute_plan requires 1 or 2 arguments".to_string(),
        stack: vec![],
    })
}
    ...
    if let Value::Object(obj_rc) = v {
        ...
        let op = obj
            ...
            .ok_or_else(|| EvalError {
                message: "Plan node missing '_op'".to_string(),
                stack: vec![],
            })?;

        match op {
            "p_projection" => {
                let input_val = obj.get("input").ok_or_else(|| EvalError {
                    message: "Projection missing input".to_string(),
                    stack: vec![],
                })?;
                ...
                let exprs_val = obj
                    ...
                    .ok_or_else(|| EvalError {
                        message: "Projection missing exprs array".to_string(),
                        stack: vec![],
                    })?;
            ...
            "p_filter" => {
                let input_val = obj.get("input").ok_or_else(|| EvalError {
                    message: "Filter missing input".to_string(),
                    stack: vec![],
                })?;
                let input = Box::new(value_to_logical_plan(input_val)?);
                let pred_val = obj.get("predicate").ok_or_else(|| EvalError {
                    message: "Filter missing predicate".to_string(),
                    stack: vec![],
                })?;
            ...
            "p_scan" => {
                let id = obj
                    ...
                    .ok_or_else(|| EvalError {
                        message: "Scan missing source_id".to_string(),
                        stack: vec![],
                    })?;
                ...
                if let Some(Value::Object(opt_rc)) = obj.get("options") {
                    ...
                    for (k, v) in opt_map.iter() {
                        if let Value::Str(s) = v {
                            res_map.insert(k.clone(), s.clone());
                        }
                    }
            ...
            "p_aggregate" => {
                let input_val = obj.get("input").ok_or_else(|| EvalError {
                    message: "Aggregate missing input".to_string(),
                    stack: vec![],
                })?;
                let input = Box::new(value_to_logical_plan(input_val)?);
                let keys_val = obj
                    ...
                    .ok_or_else(|| EvalError {
                        message: "Aggregate missing keys array".to_string(),
                        stack: vec![],
                    })?;
                let aggs_val = obj
                    .get("aggs")
                    ...
                    .ok_or_else(|| EvalError {
                        message: "Aggregate missing aggs array".to_string(),
                        stack: vec![],
                    })?;
                ...
                for a in aggs_val.read().unwrap_or_else(|e| e.into_inner()).iter() {
                    ...
                    if let Value::Object(obj_rc) = a {
                        ...
                        if kind == "agg" {
                            ...
                            let args = obj.get("_args").and_then(|v| match v {
                            ...
                            if let Some(ar_rc) = args {
                                ...
                                if !ar.is_empty() {
                                    if let Value::Object(inner_rc) = &ar[0] {
                                        let inner =
                                            inner_rc.read().unwrap_or_else(|e| e.into_inner());
            ...
            "p_join" => {
                let left_val = obj.get("left").ok_or_else(|| EvalError {
                    message: "Join missing left".to_string(),
                    stack: vec![],
                })?;
                let right_val = obj.get("right").ok_or_else(|| EvalError {
                    message: "Join missing right".to_string(),
                    stack: vec![],
                })?;
                ...
                Ok(LogicalPlan::Join {
                    ...
                    on_left,
                    on_right,
                    join_type: df_engine::JoinType::Inner,
                })
            }
            "p_sort" => {
                let input_val = obj.get("input").ok_or_else(|| EvalError {
                    message: "Sort missing input".to_string(),
                    stack: vec![],
                })?;
                let input = Box::new(value_to_logical_plan(input_val)?);
                let col = obj
            ...
            "p_limit" => {
                let input_val = obj.get("input").ok_or_else(|| EvalError {
                    message: "Limit missing input".to_string(),
                    stack: vec![],
                })?;
                let input = Box::new(value_to_logical_plan(input_val)?);
                let n = obj
            ...
            _ => Err(EvalError {
                message: format!("Unknown logical op: {}", op),
                stack: vec![],
            }),
        }
    } else {
        Err(EvalError {
            message: "Plan node must be an object".to_string(),
            stack: vec![],
        })
    }
}
...
fn value_to_named_expr(v: &Value) -> Result<(Expr, String), EvalError> {
    ...
    if let Value::Object(obj_rc) = v {
        ...
        let kind = obj
            ...
            .ok_or_else(|| EvalError {
                message: "Expr node missing '_kind'".to_string(),
                stack: vec![],
            })?;
        ...
        if kind == "alias" {
            ...
            let args_val = obj
                ...
                .ok_or_else(|| EvalError {
                    message: "alias missing args".to_string(),
                    stack: vec![],
                })?;
            let args = args_val.read().unwrap_or_else(|e| e.into_inner());
            if args.is_empty() {
                return Err(EvalError {
                    message: "alias requires 1 arg".to_string(),
                    stack: vec![],
                });
            }
...
fn value_to_expr(v: &Value) -> Result<Expr, EvalError> {
    ...
    if let Value::Object(obj_rc) = v {
        ...
        match kind {
            "col" => {
                let name = obj
                    ...
                    .ok_or_else(|| EvalError {
                        message: "col expr missing name".to_string(),
                        stack: vec![],
                    })?;
                Ok(Expr::ident(name))
            }
            "lit" => {
                let val = obj.get("_value").ok_or_else(|| EvalError {
                    message: "lit expr missing value".to_string(),
                    stack: vec![],
                })?;
                match val {
                    ...
                    _ => Err(EvalError {
                        message: "Unsupported literal value".to_string(),
                        stack: vec![],
                    }),
                }
            }
            "binop" => {
                let op_str = obj
                    ...
                    .ok_or_else(|| EvalError {
                        message: "binop missing op string".to_string(),
                        stack: vec![],
                    })?;
                ...
                let args_val = obj
                    ...
                    .ok_or_else(|| EvalError {
                        message: "binop missing args".to_string(),
                        stack: vec![],
                    })?;
                let args = args_val.read().unwrap_or_else(|e| e.into_inner());
                if args.len() != 2 {
                    return Err(EvalError {
                        message: "binop must have 2 args".to_string(),
                        stack: vec![],
                    });
                }
            ...
            "agg" | "alias" | "cast" | "unary" | "fill_null" => {
                let args_val = obj
                    ...
                    .ok_or_else(|| EvalError {
                        message: "expr missing args".to_string(),
                        stack: vec![],
                    })?;
                let args = args_val.read().unwrap_or_else(|e| e.into_inner());
                if args.is_empty() {
                    return Err(EvalError {
                        message: "expr must have at least 1 arg".to_string(),
                        stack: vec![],
                    });
                }
            ...
            _ => Err(EvalError {
                message: format!("Unsupported expr kind: {}", kind),
                stack: vec![],
            }),
        }
    } else {
        match v {
            ...
            Value::Str(s) => Ok(Expr::string(s.clone())),
            _ => Err(EvalError {
                message: "Invalid expr value".to_string(),
                stack: vec![],
            }),
        }
    }
...
fn df_to_data_chunk(df: &HashMap<String, Value>) -> Result<DataChunk, EvalError> {
    ...
    let columns_val = df
        ...
        .ok_or_else(|| EvalError {
            message: "DataFrame missing _columns".to_string(),
            stack: vec![],
        })?;
    ...
    for c_val in cols.iter() {
        if let Value::Object(c_rc) = c_val {
            ...
            let data = match c.get("_data") {
                ...
                _ => {
                    return Err(EvalError {
                        message: format!("Column '{}' missing _data or incorrect type", name),
                        stack: vec![],
                    })
                }
            };
...
fn data_chunks_to_dataframe(chunks: Vec<DataChunk>) -> Result<Value, EvalError> {
    ...
    for col in &chunk.columns {
        ...
        nyx_c.insert(
            "_data".to_string(),
            Value::Array(Arc::new(RwLock::new(vals))),
        );
    ...
    df.insert(
        "_columns".to_string(),
        Value::Array(Arc::new(RwLock::new(nyx_cols))),
    );
...
pub fn scan_parquet_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    ...
    Err(EvalError {
        message: "scan_parquet expects a path string".to_string(),
        stack: vec![],
    })
}
        ...
        for c_val in cols.iter() {
            if let Value::Object(c_rc) = c_val {
                ...
                let data_val = c
                    ...
                    .ok_or_else(|| EvalError {
                        message: "Column missing data".to_string(),
                        stack: vec![],
                    })?;
...
pub fn write_json_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    ...
    let columns = if let Value::Object(obj) = df_obj {
        ...
        if let Some(Value::Array(cols)) = obj_read.get("_columns") {
            ...
            return Err(EvalError {
                message: "Invalid DataFrame object: _columns missing".to_string(),
                stack: vec![],
            });
        }
    } else {
        return Err(EvalError {
            message: "Expected DataFrame object".to_string(),
            stack: vec![],
        });
    };
    ...
    let mut file = std::fs::File::create(&path).map_err(|e| EvalError {
        message: e.to_string(),
        stack: vec![],
    })?;
    use std::io::Write;
    ...
    for i in 0..num_rows {
        ...
        let json_str = serde_json::to_string(&row_obj).map_err(|e| EvalError {
            message: e.to_string(),
            stack: vec![],
        })?;
        writeln!(file, "{}", json_str).map_err(|e| EvalError {
            message: e.to_string(),
            stack: vec![],
        })?;
    }
...
pub fn resample_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let Some(Value::Array(data)) = args.first() {
        let bucket_size = match args.get(1) {
            Some(Value::Int(i)) => (*i as usize).max(1),
            _ => 1,
        };
        let agg_op = match args.get(2) {
            ...
            if let Value::Object(c_rc) = c_val {
                ...
                if name == col_name {
                    let data_val = c
                        .get("_data")
                        .and_then(|v| match v {
                            Value::Array(a) => Some(a.read().unwrap_or_else(|e| e.into_inner())),
                            _ => None,
                        })
                        .ok_or_else(|| EvalError {
                            message: "Column missing data".to_string(),
...
pub fn from_tensor_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let Some(Value::Object(tensor_rc)) = args.first() {
        ...
        let cols_count = match shape.get(1) {
            Some(Value::Int(i)) => *i as usize,
            _ => return Ok(Value::Null),
        };
        let flat = data_arr.read().unwrap_or_else(|e| e.into_inner());

        let nyx_cols: Vec<Value> = (0..cols_count)
            .map(|c| {
                ...
                let col_data: Vec<Value> = (0..rows)
                    .map(|r| flat.get(r * cols_count + c).cloned().unwrap_or(Value::Null))
                    .collect();
                ...
                col_map.insert(
                    "_data".to_string(),
                    Value::Array(Arc::new(RwLock::new(col_data))),
                );
                Value::Object(Arc::new(RwLock::new(col_map)))
            })
            .collect();
...
pub fn export_arrow_ipc_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let Some(Value::Object(df_rc)) = args.first() {
        ...
        let num_rows = if let Some(Value::Object(c0)) = cols.first() {
            ...
            if let Some(Value::Array(d)) = cr.get("_data") {
...
pub fn set_memory_limit_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    ...
    println!(
        "[df] Memory limit set to {} bytes ({:.1} MB)",
        limit,
...
pub fn profile_plan_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let Some(plan_val) = args.first() {
        ...
        let mut phys =
            df_engine::create_physical_plan(&logical, &mut ctx).map_err(|e| EvalError {
                message: e,
                stack: vec![],
            })?;
        let mut chunk_count = 0i64;
        let mut row_count = 0i64;
        while let Some(chunk) = phys.next_chunk().map_err(|e| EvalError {
            message: e,
            stack: vec![],
        })? {
            chunk_count += 1;
...
pub fn sandboxed_eval_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    ...
    Err(EvalError {
        message: format!("sandboxed_eval: unsupported expression '{}'", expr_str),
        stack: vec![],
    })
}
...
pub fn df_distinct_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let Some(Value::Object(df_rc)) = args.first() {
        ...
        if cols.is_empty() {
            return Ok(Value::Object(Arc::new(RwLock::new(df.clone()))));
        }
        ...
        let new_cols: Vec<Value> = cols
            ...
            .map(|cv| {
                if let Value::Object(cr) = cv {
                    ...
                    let dtype = match col.get("_dtype") {
                        Some(Value::Str(s)) => s.clone(),
                        _ => "str".to_string(),
                    };
                    if let Some(Value::Array(da)) = col.get("_data") {
                        let d = da.read().unwrap_or_else(|e| e.into_inner());
                        let nd: Vec<Value> = kept
                            .iter()
                            .map(|&r| d.get(r).cloned().unwrap_or(Value::Null))
                            .collect();
        ...
        new_df.insert(
            "_columns".to_string(),
            Value::Array(Arc::new(RwLock::new(new_cols))),
        );
...
pub fn soft_join_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    ...
    let left_df = match &args[0] {
        Value::Object(o) => o.read().unwrap_or_else(|e| e.into_inner()),
        _ => return Ok(Value::Null),
    };
    ...
    for r_col_val in right_cols.iter() {
        if let Value::Object(c_rc) = r_col_val {
            ...
            for l_id in &left_ids {
                ...
                new_data.push(Value::Float(if weight_total > 0.0 {
            ...
            new_col.insert(
                "_data".to_string(),
                Value::Array(Arc::new(RwLock::new(new_data))),
            );
    ...
    result_df.insert(
        "_columns".to_string(),
        Value::Array(Arc::new(RwLock::new(joined_cols))),
    );
...
pub fn db_commit_transaction_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let Some(Value::Int(tx_id)) = args.first() {
        let pending = df_engine::global_tx_context()
            .commit(*tx_id as u64)
            .map_err(|e| EvalError {
                message: e,
                stack: vec![],
...
pub fn save_table_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let (Some(Value::Str(name)), Some(Value::Str(path))) = (args.first(), args.get(1)) {
        ...
        if let Some(chunks) = catalog.get(name) {
            ...
            crate::runtime::execution::nyx_table_writer::NyxTableWriter::write_to_file(
                path, &schema, chunks,
            )
            .map_err(|e| EvalError {
                message: e.to_string(),
...
pub fn load_table_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let (Some(Value::Str(name)), Some(Value::Str(path))) = (args.first(), args.get(1)) {
        let (_schema, chunks) =
            crate::runtime::execution::nyx_table_writer::NyxTableWriter::read_from_file(path)
                .map_err(|e| EvalError {
                    message: e.to_string(),
                    stack: vec![],
                })?;
...
pub fn db_add_pending_table_native(_vm: &mut NyxVm, args: &[Value]) -> Result<Value, EvalError> {
    if let (Some(Value::Int(tx_id)), Some(Value::Str(name)), Some(Value::Object(df_rc))) =
        (args.first(), args.get(1), args.get(2))
    {
        ...
        df_engine::global_tx_context()
            .add_pending_table(*tx_id as u64, name.clone(), schema, vec![chunk])
            .map_err(|e| EvalError {
                message: e,
                stack: vec![],
            })?;

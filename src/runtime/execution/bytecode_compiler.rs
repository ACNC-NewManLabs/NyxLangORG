use crate::core::ast::ast_nodes::{Expr, ItemKind, Program, Stmt};
use nyx_vm::bytecode::{BytecodeModule, OpCode, Value};
...
impl BytecodeCompiler {
    ...
    pub fn compile_loop_fragment(
        ...
        let _loop_func_idx = self
            .emitter
            .create_function("jit_loop", external_locals.len());
            ...
            Expr::Call { callee, args, span } => {
                ...
                if let Expr::Identifier { name, .. } = &**callee {
                    ...
                    self.emitter.emit_instr(
                        OpCode::CallExt,
                        vec![name_idx as i32, num_args as i32],
                        span.start.line,
                    );
                    Ok(())
                } else {
    ...
    pub fn compile_stmt(&mut self, stmt: &Stmt) -> Result<(), String> {
        match stmt {
            Stmt::Expr(expr) => {
                ...
                self.emitter
                    .emit_instr(OpCode::POP, vec![], expr.span().start.line);
                Ok(())
            }
            Stmt::Let {
                name, expr, span, ..
            ...
            Stmt::If {
                ...
                for br in branches {
                    ...
                    end_jumps
                        .push(self.emit_placeholder(OpCode::JMP, br.condition.span().end.line));
            ...
            Stmt::Print { expr } => {
                ...
                let name_idx = self
                    .emitter
                    .add_constant(Value::String("print".to_string()));
                self.emitter.emit_instr(
                    OpCode::CallExt,
    ...
    fn get_current_instr_idx(&self) -> usize {
        self.emitter.current_instr_idx()
    }
    ...
    fn patch_instr(&mut self, instr_idx: usize, target: usize) {
        self.emitter
            .patch_instr_operand(instr_idx, 0, target as i32);
    }
}

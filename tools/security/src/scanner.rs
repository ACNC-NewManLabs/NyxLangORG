...
impl SecurityScanner {
    ...
    pub fn new() -> Self {
        let vulnerability_patterns = vec![
            ...
            VulnerabilityPattern {
                ...
                recommendation: "Avoid using user input in system calls. Use parameterized commands.",
            },
            // Path Traversal
            VulnerabilityPattern {
    ...
    fn scan_ast(&self, content: &str, file_path: &Path, vulnerabilities: &mut Vec<Vulnerability>, secrets: &mut Vec<Secret>) {
        ...
        if let Ok(tokens) = lexer.tokenize() {
            ...
            for tok in &tokens {
                ...
                if tok.kind == TokenKind::Comment || tok.kind == TokenKind::MultiLineComment {
                     ...
                     if Regex::new(r"#\s*TODO.*(?:hack|fixme|bug|issue|problem)").unwrap().is_match(&tok.lexeme) {
                         vulnerabilities.push(Vulnerability {
                            ...
                            description: "Incomplete TODO comment suggesting a known issue".to_string(),
    ...
    fn scan_stmts(&self, stmts: &[Stmt], file_path: &Path, vulnerabilities: &mut Vec<Vulnerability>) {
        for stmt in stmts {
            match stmt {
    ...
    fn scan_expr(&self, expr: &Expr, file_path: &Path, vulnerabilities: &mut Vec<Vulnerability>) {
         ...
         match expr {
                      ...
                      for pattern in &self.vulnerability_patterns {
                          if pattern.pattern.is_match(ident) {
                             let description = if let Some(info) = self.cwe_database.get(pattern.cwe_id) {
                                 format!("{}: {}. {}", info.name, info.description, pattern.description)
                             } else {
             ...
             Expr::Match { expr: mat_expr, arms, .. } => {
                  self.scan_expr(mat_expr, file_path, vulnerabilities);
                  for arm in arms {
                       match &arm.body {
                           nyx::core::ast::ast_nodes::MatchBody::Expr(e) => self.scan_expr(e, file_path, vulnerabilities),
                           nyx::core::ast::ast_nodes::MatchBody::Stmt(s) => self.scan_stmts(&[s.clone()], file_path, vulnerabilities),
                           nyx::core::ast::ast_nodes::MatchBody::Block(b) => self.scan_stmts(b, file_path, vulnerabilities),
                       }
                  }
             }
             _ => {}
    ...
    pub fn scan_dependencies(&self, path: &Path) -> Result<Vec<Vulnerability>, String> {
        ...
        if packages_json.exists() {
            ...
            let vulnerable_packages = [
                ("old-package", "CWE-1104", "Abandoned Package", "Use maintained alternative"),
            ];

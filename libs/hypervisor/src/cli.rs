...
impl Cli {
    ...
    pub fn execute(&self) -> Result<(), String> {
        match &self.command {
            Command::Run { kernel, cpus, memory, arch, accel, iso } => {
                let config = HypervisorConfig {
                    num_cpus: *cpus,
                    memory_mb: *memory,
                    kernel_path: kernel.as_ref().map(|p| p.to_string_lossy().to_string()).unwrap_or_default(),
                    iso_path: iso.as_ref().map(|p| p.to_string_lossy().to_string()).unwrap_or_default(),

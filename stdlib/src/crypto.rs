...
pub mod crypto {
    ...
    pub mod cipher {
            ...
            let ciphertext = match key.len() {
                AES128_KEY_LEN => {
                    let cipher = Aes128Gcm::new_from_slice(key).map_err(|_| {
                        NyxError::new("CRYP001", "Invalid AES-128 key", ErrorCategory::Security)
                    })?;
                    cipher
                ...
                AES256_KEY_LEN => {
                    let cipher = Aes256Gcm::new_from_slice(key).map_err(|_| {
                        NyxError::new("CRYP003", "Invalid AES-256 key", ErrorCategory::Security)
                    })?;
                    cipher
                        .encrypt(Nonce::from_slice(&nonce), data)
                        .map_err(|_| {
        ...
        pub fn chacha_encrypt(data: &[u8], key: &[u8]) -> Result<NyxVec<u8>, NyxError> {
            ...
            let cipher = ChaCha20Poly1305::new_from_slice(key).map_err(|_| {
                NyxError::new("CRYP018", "Invalid ChaCha20 key", ErrorCategory::Security)
            })?;
        ...
        pub fn chacha_decrypt(data: &[u8], key: &[u8]) -> Result<NyxVec<u8>, NyxError> {
            ...
            let cipher = ChaCha20Poly1305::new_from_slice(&k).map_err(|_| {
                NyxError::new("CRYP018", "Invalid ChaCha20 key", ErrorCategory::Security)
            })?;
            let res = cipher
                .decrypt(Nonce::from_slice(nonce), ciphertext)
        ...
        pub fn chacha_encrypt_with_nonce(
            data: &[u8],
            key: &[u8],
            nonce: &[u8],
        ) -> Result<NyxVec<u8>, NyxError> {
            if key.len() != CHACHA_KEY_LEN {
                return Err(NyxError::new(
                    "CRYP017",
                    "Invalid ChaCha20 key length",
        ...
        pub fn chacha_decrypt_with_nonce(
            data: &[u8],
            key: &[u8],
            nonce: &[u8],
        ) -> Result<NyxVec<u8>, NyxError> {
    ...
    pub mod signature {
        ...
        pub fn sign(data: &[u8], key: &[u8]) -> Result<NyxVec<u8>, NyxError> {
            let signing_key = match key.len() {
                32 => SigningKey::from_bytes(key.try_into().map_err(|_| {
                    NyxError::new(
                        "CRYP006",
                ...
                64 => SigningKey::from_keypair_bytes(key.try_into().map_err(|_| {
                    NyxError::new("CRYP007", "Invalid keypair length", ErrorCategory::Security)
                })?)
                .map_err(|_| {
                    NyxError::new("CRYP008", "Invalid keypair bytes", ErrorCategory::Security)
                })?,
                _ => {
                    return Err(NyxError::new(
                        "CRYP009",
                        "Signature key must be 32 or 64 bytes",
                        ErrorCategory::Security,
                    ))
                }
            };
        ...
        pub fn verify(data: &[u8], sig: &[u8], key: &[u8]) -> Result<bool, NyxError> {
            let verifying_key = match key.len() {
                32 => VerifyingKey::from_bytes(key.try_into().map_err(|_| {
                    NyxError::new(
                        "CRYP010",
    ...
    pub mod kdf {
        ...
        pub fn argon2_hash(password: &[u8], salt: &[u8]) -> Result<String, NyxError> {
            ...
            let password_hash = argon2
                ...
                .map_err(|_| {
                    NyxError::new("CRYP022", "Argon2 hashing failure", ErrorCategory::Security)
                })?
                .to_string();
            Ok(password_hash)
        }
        ...
        pub fn hkdf_expand(
            ...
            info: &[u8],
            len: usize,
        ) -> Result<StdVec<u8>, NyxError> {
            let hk = Hkdf::<Sha256>::new(salt, ikm);
            let mut okm = vec![0u8; len];
            hk.expand(info, &mut okm).map_err(|_| {
                NyxError::new("CRYP023", "HKDF expansion failure", ErrorCategory::Security)
            })?;
            Ok(okm)
        }
    ...
    pub mod key_exchange {
        ...
        pub fn generate_x25519_keypair() -> (SecretKey, SecretBytes) {
            ...
            (
                ...
                SecretBytes(public.as_bytes().to_vec()),
            )
        }

        pub fn diffie_hellman(secret: &[u8], public: &[u8]) -> Result<SecretBytes, NyxError> {
            let s_bytes: [u8; 32] = secret.try_into().map_err(|_| {
                NyxError::new(
                    "CRYP024",
                    "Invalid X25519 secret key length",
                    ErrorCategory::Security,
                )
            })?;
            let p_bytes: [u8; 32] = public.try_into().map_err(|_| {
                NyxError::new(
                    "CRYP025",
    ...
    pub mod encoding {
        ...
        pub fn from_base64(encoded: &str) -> Result<NyxVec<u8>, NyxError> {
            let data = general_purpose::STANDARD.decode(encoded).map_err(|_| {
                NyxError::new("CRYP027", "Invalid Base64 string", ErrorCategory::Security)
            })?;
        ...
        pub fn from_hex(encoded: &str) -> Result<NyxVec<u8>, NyxError> {
            let data = hex::decode(encoded).map_err(|_| {
                NyxError::new("CRYP028", "Invalid Hex string", ErrorCategory::Security)
            })?;
            let mut v = NyxVec::with_capacity(data.len());
            for b in data {
                v.push(b);
    ...
    pub mod seal {
        ...
        pub fn seal(message: &[u8], password: &[u8]) -> Result<NyxVec<u8>, NyxError> {
            ...
            let salt_str = SaltString::encode_b64(&salt_bytes).map_err(|_| {
                NyxError::new("SEAL001", "Salt encoding failed", ErrorCategory::Security)
            })?;
            ...
            let mut key = [0u8; 32];
            argon2
                .hash_password_into(password, salt_str.as_salt().as_ref().as_bytes(), &mut key)
                .map_err(|_| {
                    NyxError::new("SEAL002", "Key derivation failed", ErrorCategory::Security)
                })?;
            ...
            let mut out = NyxVec::with_capacity(
                MAGIC.len() + 1 + 1 + SALT_LEN + NONCE_LEN + ciphertext.len(),
            );
            for b in MAGIC {
                out.push(*b);
            }
        ...
        pub fn seal_ephemeral(
            ...
            let mut session_key =
                kdf::hkdf_expand(shared_secret.as_slice(), None, b"nyx_ephemeral_session", 32)?;
            let nonce = random::random_bytes(12);
            let encrypted = cipher::chacha_encrypt_with_nonce(
                message,
